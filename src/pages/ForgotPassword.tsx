...
const ForgotPassword: React.FC = () => {
  ...
  // GSAP animations for success/error message
  useEffect(() => {
    if ((status === 'success' || status === 'error') && messageRef.current) {
      const messageText = messageRef.current.querySelector('p');
      if (messageText) {
        // Split text into characters
        const chars = messageText.innerText.split('');
        messageText.innerHTML = chars
          .map((char) => `<span class="char">${char}</span>`)
          .join('');

        // Character-by-character animation with wave effect
        gsap.fromTo(
          messageRef.current.querySelectorAll('.char'),
        ...
        gsap.to(messageRef.current, {
          boxShadow: `0 0 20px rgba(${
            status === 'success' ? '74, 222, 128' : '239, 68, 68'
          }, 0.5), 0 0 40px rgba(${status === 'success' ? '74, 222, 128' : '239, 68, 68'}, 0.3)`,
          duration: 0.5,
          repeat: 5,
          yoyo: true,
          ease: "power2.inOut",
        ...
        gsap.to(messageText, {
          textShadow: `0 0 10px rgba(${
            status === 'success' ? '74, 222, 128' : '239, 68, 68'
          }, 0.8), 0 0 20px rgba(${status === 'success' ? '74, 222, 128' : '239, 68, 68'}, 0.5)`,
          duration: 0.3,
          repeat: 3,
          yoyo: true,
          ease: "power2.inOut",
        ...
        // Particle effect for success/error
        const particleCount = 30;
        const particlesContainer = document.createElement('div');
        particlesContainer.className = 'particles absolute inset-0 pointer-events-none';
        messageRef.current.appendChild(particlesContainer);
        ...
        >
          <motion.div variants={inputVariants} initial="hidden" animate="visible">
            ...
            <motion.input
              ...
              } placeholder-gray-400 text-gray-900 focus:outline-none focus:ring-0 sm:text-sm bg-white/50 backdrop-blur-sm`}
            ...
            {(status === 'success' || status === 'error') && (
              <motion.div
                ...
                className={`rounded-xl p-4 relative overflow-hidden border ${
                  status === 'success'
                    ? 'bg-green-50/90 backdrop-blur-sm border-green-200/50'
                    : 'bg-red-50/90 backdrop-blur-sm border-red-200/50'
                }`}
              >
